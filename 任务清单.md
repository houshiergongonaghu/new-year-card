...
## Phase 6: 数据存储与邮件发送
- [x] 创建存储元数据 API action (`/api/save/card`)
- [x] 写入 cards 表 (Supabase 数据库)
- [x] 集成 Resend API (代码实现，需配置 API Key)
- [x] 实现邮件模板 (HTML 模板，包含贺卡预览和 CTA 按钮)
- [x] 前端集成 (app/page.tsx handleSubmit 函数)
- [x] 获取 RESEND_API_KEY 并配置到 .env.local
- [x] 使用 Resend 测试域名 (onboarding@resend.dev) 发送邮件（方案A）
- [ ] 测试邮件发送功能（需要重启服务器）

## Phase 7: 查看页与病毒传播
- [x] 创建 /card/[id] 页面 (app/card/[id]/page.tsx)
- [x] 复用节日主题 CSS (粉紫色渐变背景)
- [x] 居中显示最终贺卡大图 (max-w-2xl 居中)
- [x] 添加 "我也要做一张" CTA 按钮 (粉色渐变按钮)
- [x] 点击 CTA 跳转回首页 (Link to "/")
- [ ] 测试完整流程
...
### Phase 4: AI 生成功能 - 后续工作
**当前使用 MOCK_AI 模式**
- [x] 增加 MOCK_AI=1 环境变量支持
- [x] /api/generate 检测该开关，直接返回本地图片 URL
- [x] 保留限流和日志功能
- [ ] 充值 Replicate 后删除 .env.local 里的 MOCK_AI=1 或改为 0
- [ ] 重启 npm run dev 即可使用真实 AI 生成
- [ ] 按正确流程改造：先上传用户图到 Supabase，再用 URL 调用 Replicate，生成后下载并重新上传 Supabase

### Phase 6: 邮件发送 - 后续工作
**当前使用 Resend 测试域名**
- [x] 使用 onboarding@resend.dev 测试域名（方案A）
- [x] 只能发送到注册邮箱（daisyquanzhixian@gmail.com）
- [x] 邮件主题带 [TEST] 标记
- [x] 用于验证整个流程是否正常工作

**阶段2: 验证域名 (Phase 7-8 之间)**
- [ ] 访问 https://resend.com/domains
- [ ] 添加并验证你的域名（需DNS配置）
- [ ] 修改 app/api/send/email/route.ts:162，从 onboarding@resend.dev 改为 noreply@yourdomain.com
